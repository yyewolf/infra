...
- **gVisor runs on cp-2 only** — `siderolabs/gvisor` plus `patches/gvisor.yaml`, which sets `user.max_user_namespaces` (Talos ships it at 0; runsc cannot start without it) and registers a `runsc-netraw` handler running runsc with `--net-raw` and `--allow-packet-socket-write` for Docker-in-pod. The `gvisor` RuntimeClass (`flux/infrastructure/gvisor`) pins to `cp-2` by hostname. The extension's own flagless `runsc` handler is deliberately left unexposed.
