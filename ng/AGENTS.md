...
- **The proxies' network policy is the loose one, and it has to be** — `tailscale-operator` (selector `app: operator`) only needs the apiserver, DNS and three named `*.tailscale.com` hosts on 443. `tailscale-proxies` (selector `tailscale.com/managed: "true"`, the only label a proxy carries that does not name the Service it was created for) additionally needs UDP to `world` on every port: STUN and then WireGuard direct to whatever ip:port the peer turns out to be behind, neither knowable in advance. Restricting it does not break the tunnel, it silently demotes it to relaying every byte through DERP. The proxies also talk to the apiserver because tailscaled keeps its node state in a Secret rather than on disk.
