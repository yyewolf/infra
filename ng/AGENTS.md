...
- **MySQL backs up to Infomaniak Swiss Backup; Postgres still does not** — `platform/mysql` has a `backup.objectStore` pointing at `https://s3.swiss-backup02.infomaniak.com`, bucket `default`, prefix `/mysql`, region `us-east-1`, path-style addressing, with credentials from the SOPS-encrypted `mysql-backup-s3` Secret. `continuousArchiving` ships binlogs (`targetRPOSeconds: 300`) and a `ScheduledBackup` takes a nightly xtrabackup base backup from the standby, 30d retention on both. Postgres would take `/postgres` in the same bucket — the bucket is not MySQL-specific, only the prefix is.
